...
//   · 영상은 글당 1개 — 30MB · 60초 안(저장공간이 1GB라 짧게). 넘으면 안내만 하고 안 받는다
...
export default function PostComposer({
  ...
  const pickVideo = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (f.size > MAX_VIDEO_MB * 1024 * 1024) {
      onNotice?.(`영상은 ${MAX_VIDEO_MB}MB 안으로 올려주세요`); return
    }
    // 길이 확인 — 브라우저가 메타데이터만 읽는다(전체를 올리지 않음)
    ...
    if (sec > MAX_VIDEO_SEC) { onNotice?.(`영상은 ${MAX_VIDEO_SEC}초 안으로 올려주세요`); return }
    onVideoChange(f)
  }
    ...
    <div className="rounded-card border border-rule bg-paper p-4">
      ...
      <div className="flex items-center justify-between mt-3 pt-3 border-t border-rule flex-wrap gap-2">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button type="button" onClick={() => videoRef.current?.click()} disabled={hasVideo || checkingVideo} className={btn}>
            <span aria-hidden="true">🎬</span> {checkingVideo ? '확인 중…' : '영상'}
          </button>
      ...
      <p className="text-[11px] text-ink-faint mt-2">영상은 1개, {MAX_VIDEO_SEC}초·{MAX_VIDEO_MB}MB 안으로 짧게</p>
