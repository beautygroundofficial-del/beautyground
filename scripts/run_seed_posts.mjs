#!/usr/bin/env node
// 시딩 게시물 일괄 등록 — 2026-09-17 (v2: photo_type 통과 + 결과 자동 저장)
// seed_posts_plan.json(계정별 email/password/nickname/category/content/image/photo_type)을 읽어
// 각 계정으로 로그인 → 이미지 업로드 → create_board_post 호출을 순서대로 실행한다.
// plan 파일엔 비밀번호가 평문으로 들어가므로 저장소에 커밋하지 말 것(스크래치패드에만 보관).
//
// plan 항목의 photo_type — 사진이 어떤 종류인지 태그(gen_seed_reactions.mjs가 이걸 보고 댓글 문구를 고름):
//   scenery(풍경·자연·여행지) / info(간판·칠판·메뉴판 등 텍스트/정보 위주) / mood(골목·인물뒷모습 등 분위기) / object(사물·소품)
//
// 사용: node --env-file=.env scripts/run_seed_posts.mjs <plan.json> [결과저장경로.json]
...
const outPath = process.argv[3] ?? planPath.replace(/\.json$/, '.result.json')
if (!planPath) { console.error('사용: node --env-file=.env scripts/run_seed_posts.mjs <plan.json> [결과저장경로.json]'); process.exit(2) }
...
for (const item of plan) {
  ...
  results.push({ email: item.email, ok: true, category: item.category, nickname: item.nickname, photo_type: item.photo_type ?? 'scenery', post })
